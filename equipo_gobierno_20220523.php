   ...
   <?php    
    ...
    switch(@$_REQUEST['accion'])
    {
        case 'detalles':
            ...
            break; 
        case 'alta':
            alta($Sevius,$conexion,$tipo,$_REQUEST['dni'],@$_REQUEST['cargo']);
            break;
        case 'altaX':
            alta($Sevius,$conexion,$tipo,$_REQUEST['dni'],@$_REQUEST['cargo']);
            break;        
    ...
    if ($n==0){
        ...
        <?php echo'<table class="tablaListado">';
            ...
            for($i=0;$i<$n;$i++){
                ...
                echo "<td><button id='detalles".$i ."' name='detalles" .$i ."'>Detalles</button></td>";
                echo "<td><button id='alta".$i ."' name='alta" .$i ."'>Alta</button></td></tr>";
                $detalles='detalles'.$i;
...
?>
    ...
    {
    /* Esto saca los elementos de un formulario
    var formularios = document.acciones;
       var msg = "";
       for (var i=0; i<formularios.elements.length;i++)
       {
          //msg = msg + '\n\nElemento '+j+ ' del formulario '+(i+1)+ ' tiene id: '+ formularios[i].elements[j].id;
          msg = msg + ' y name: ' + formularios.elements[i].name;
       }
      alert (msg);
      */
      if (acc == 'altaX')
       { 
       //alert(document.acciones.xdni.value);
       document.acciones.dni.value=document.acciones.xdni.value; 
       }
       else
       {
...
function alta($Sevius,$conexion,$tipo,$dni,$cargo)
{
    echo '<BR />alta ' .$tipo .' -- ' .$dni.'<BR />';
    list($n,$t)=datos_alta_dni($Sevius,$conexion,$tipo,$dni);
    
    ?>
    <table class="tablaListado">
    <caption>Datos del nuevo cargo</caption>
    <tr><td>Introduzca DNI</td>
        <td><input type="text" id="xdni" name="xdni" value='' /></td>
        <td>Cargo:</td>
        <td><input type="text" id="cargo" value="Esto se rellenará sólo según el cargo sobre el que haya pulsado el botón de alta<?php echo $conexion->data[$i]['DESCRIPCION_CARGO']?>" readonly></td>
        <td>Depende de:</td>
        <td>Desplegable con los cargos:</td>
        <td><button id="validar" name="validar" >Validar DNI</button></td>
        <?php 
        $validar='validar';
        $dni=$_REQUEST['dni'];
        $Sevius->jsEvent($validar,'click',"funcase($tipo,'altaX','',$dni);");
        ?>
    </tr>
    <tr><td>Nombre:</td>
        <td><input type="text" id="nombre" value="<?php if ($dni=='') echo ""; else echo $t[0]['NOMLEGAL']; ?>" readonly></td>
        <td>Apellido 1:</td>
        <td><input type="text" id="apell1" value="<?php if ($dni=='') echo ""; else echo $t[0]['LL1PRS']; ?>" readonly></td>
        <td>Apellido 2:</td>
        <td><input type="text" id="apell2" value="<?php if ($dni=='') echo ""; else echo $t[0]['LL2PRS']; ?>" readonly></td>
        <td><button id="aceptar" name="aceptar">Aceptar</button></td>
    </tr>
    </table>
    <?php
...
function datos_alta_dni($Sevius,$conexion,$tipo,$dni)
{
    echo 'El DNI es: '.$dni;
    $conexion->parse('select nomlegal,ll1prs, ll2prs 
                    from tuib_persona 
                    where dniprs =:dni',true);
