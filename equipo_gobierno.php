...
echo'<table class="tablaListado">';
    ...
    $conexion->parse('select codnum, descripcion_cargo, nombre, apellido1, apellido2, fecha_alta, fecha_baja from cydeg_personahistorico');
    $conexion->novalue();
    $conexion->execute();
    $conexion->fetch();
    if ($conexion->rows>0){
        echo'<tr><th>Código</th><th>Descripción</th><th>Nombre</th><th>Apellido1</th><th>Apellido2</th><th>Fecha alta</th><th>Fecha baja</th></tr>';
        for($i=0;$i<$conexion->rows;$i++){
            ...
            echo'<td><input type="text" id="f_baja" value="'.$conexion->data[$i]['FECHA_BAJA'].'" readonly></td></tr>';
            //echo'<td><input type="submit" id="actualizar" value="Actualizar"></td></tr>';
